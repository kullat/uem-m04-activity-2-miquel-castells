...
BUCKET_NAME = 'uem-m04-activity-2-miquel-castells-kirin'
DB_NAME = 'models-db'
CONFIG_FILE = 'model_config'

def training_pipeline(path, model_info_db_name=DB_NAME):
    """
    ...
    model_config = load_model_config(model_info_db_name)[CONFIG_FILE]
...
def load_model_config(db_name):
    ...
    """
    db = client.get_database(db_name)
    query = Query(db, selector={'_id': {'$eq': CONFIG_FILE}})
    return query()['docs'][0]
